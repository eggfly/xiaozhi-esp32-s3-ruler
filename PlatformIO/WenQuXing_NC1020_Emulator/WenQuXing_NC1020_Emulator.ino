...
//std::map<std::string, char> nc1020_keymap = {
//  {"1", '1'},
//  {"2", '2'},
//  {"3", '3'},
//  {"4", '4'},
...
const float PROGMEM backlight_levels[] = {
  0.0, 0.02, 0.05, 0.15, 0.3, 0.5, 0.75, 1.0,
};
...
uint8_t func_keys[] = {8, 8, 9, 9, 10, 10, 11, 11, 12, 12, 13, 13, 14, 14,};
...
inline void nc1020_loop() {
  ...
  canvas.fillScreen(0x0); // WHITE
  canvas.drawBitmap(
    SCREEN_WIDTH / 2 - 320 / 2,
    SCREEN_HEIGHT / 2 - 160 / 2,
    lcd_buff_expanded, 320, 160, WQX_COLOR);

  canvas.drawBitmap(0, 0, lcd_buff, 160, 80, WQX_COLOR);
  // canvas.drawBitmap(160, 0, lcd_buff, 160, 80, WQX_COLOR);
  canvas.drawBitmap(320, 0, lcd_buff, 160, 80, WQX_COLOR);

  canvas.drawBitmap(0, 240, lcd_buff, 160, 80, WQX_COLOR);
  // canvas.drawBitmap(160, 240, lcd_buff, 160, 80, WQX_COLOR);
  canvas.drawBitmap(320, 240, lcd_buff, 160, 80, WQX_COLOR);

  if (key_to_release > 0) {
    if (key_release_countdown == 0) {
      wqx::SetKey(key_to_release, false);
      // Serial.printf("Release key: %d\n", key_to_release);
      key_to_release = -1;
    } else {
    ...
    char key = 0;
    if (xQueueReceive(g_event_queue_handle, &key, (TickType_t)0) == pdTRUE) {}

    if (key != 0) {
      int keycode = 0;
      if (cardkb_to_nc1020_keymap.find(key) != cardkb_to_nc1020_keymap.end()) {
        keycode = cardkb_to_nc1020_keymap[key];
        Serial.printf("xQueueReceive, key=0x%02x,' % c'->0x%02x\n", key, key, keycode);
      } else {
        Serial.printf("xQueueReceive, key=0x%02x,' % c'-> not found!\n", key, key);
        keycode = -1;
      }
